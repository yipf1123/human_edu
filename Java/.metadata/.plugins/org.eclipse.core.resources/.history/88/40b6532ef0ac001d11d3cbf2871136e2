...
public class Main {
	...
	/**
	 * 전체 게시글 조회 메소드 - DAO 객체로 데이터 전체 조회 요청 메소드를 호출한다. - List<Board> 타입의 게시글 목록을
 ...
 */
	...
	/**
	 * 게시글 쓰기 - 제목, 작성자, 내용을 입력받아 Board 객체를 생성하고 DAO 객체의 데이터 삽입 요청 메소드를 호출한다.
	 */
	...
	/**
	 * 게시글 수정 메소드 - 수정할 게시글 번호를 입력 받는다. - 제목, 작성자, 내용을 입력받아 Board 객체를 생성한다. - DAO
	 * 객체의 데이터 수정 메소드를 호출한다.
	 */
	...
	private static void print(Board board) {
		...
		// 게시글 정보가 없으면 출력x
		if (board == null) {
			System.out.println("게시글 정보가 존해하지 않습니다");
			return;
		}
	...
	/**
	 * 게시글 목록 전체 출력 메소드 -List<Board> 타입의 게시글 전체 목록을 반복하여, 각 객체를 인자로 개별 출력 메소드를 호출한다
	 */

	private static void printAll() {

		if (boardList.isEmpty()) {
			System.out.println("존해하는 게시글이 없습니다.");
			return;
		}
