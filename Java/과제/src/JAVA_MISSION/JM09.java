...
public class JM09 {
	...
	public static void calcClassTotal(int classNo) {
		...
		for (int i = 0; i < classStudentData[classNo-1].length; i++) {
			...
			// 해당 반의 전체 총점                 // clssNo 반의 i번째 학생의 총점
			classTotalScore[classNo-1][6] += classStudentData[classNo-1][i][0];
					// 국어, 영어, 수학, 사회, 과학, 자바    전체
					//  0   1    2    3   4    5       6	
		}
		
		// 과목별 평균
		for (int i = 0; i < classTotalScore[classNo-1].length; i++) {
			classTotalAverage[classNo-1][i] 
					= Math.round((double) classTotalScore[classNo-1][i]/classCount[classNo-1]*100.0)/100.0;
		}
